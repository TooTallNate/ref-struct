...
function Struct (fields) {
  ...
  var constructor = function (arg, data) {
    ...
    if (Buffer.isBuffer(arg)) {
      ...
      this.pointer = new Buffer(constructor.size)
    }
    if (arg) {
      for (var key in arg) {
...
function defineProperty (name, type) {
  ...
  var sz  = type.indirection === 1 ? type.size : ref.sizeof.pointer
  var asz = type.alignment || sz

  //console.error('defining "%s":', name)
  ...
  function get () {
    debug('getting "%s" struct field (offset: %d)', name, offset)
    return ref.get(this.pointer, field.offset, type)
  }

  function set (value) {
    debug('setting "%s" struct field (offset: %d)', name, offset, value)
    return ref.set(this.pointer, field.offset, value, type)
  }
}
